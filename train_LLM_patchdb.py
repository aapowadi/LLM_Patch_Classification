...
data = pd.read_parquet('patchdb.parquet')
...
data['category'] = patch_type_encoder.fit_transform(data['category'])
...
def prepare_dataset(df):
    desc_embeds = get_text_embeddings(df['commit_message'].tolist())
    comp_embeds = get_text_embeddings(df['diff_code'].tolist())
    ...
    labels = torch.tensor(df['category'].values, dtype=torch.long).to(device)
    return desc_embeds, comp_embeds, labels

# Prepare training and testing datasets
train_desc_embeds, train_comp_embeds, train_labels = prepare_dataset(train_data)
test_desc_embeds, test_comp_embeds, test_labels = prepare_dataset(test_data)

# Create TensorDatasets
train_dataset = TensorDataset(train_desc_embeds, train_comp_embeds, train_labels)
test_dataset = TensorDataset(test_desc_embeds, test_comp_embeds, test_labels)
...
class PatchClassifier(nn.Module):
    def __init__(self, num_classes, d_model=768):
        ...
        self.classifier = nn.Linear(d_model * 2, num_classes)  # Combine desc, comp

    def forward(self, desc_embeds, comp_embeds):
        combined = torch.cat([desc_embeds, comp_embeds], dim=1)
...
def plot_class_f1_scores(class_names, per_class_f1, save_path='PatchDB_class_specific_f1_scores.png'):
...
def plot_conf_matrix(class_names, true_labels, predictions):
    ...
    # Save the figure with high resolution
    plt.savefig('PatchDB_confusion_matrix.png', dpi=300)
